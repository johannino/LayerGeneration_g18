...
def create_result_folders(experiment_name):
    os.makedirs("modelsjob", exist_ok=True)
    os.makedirs("resultsjob", exist_ok=True)
    os.makedirs(os.path.join("modelsjob", experiment_name), exist_ok=True)
    os.makedirs(os.path.join("resultsjob", experiment_name), exist_ok=True)

def train(device='cpu', T=500, img_size=100, input_channels=3, channels=32, time_dim=256,
          batch_size=32, lr=1e-5, num_epochs=200, experiment_name="ddpm", show=False):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, images in enumerate(pbar):
            ...
            loss =  mse(predicted_noise, noise) # line 6 from the Training algorithm
        ...
        # Save final sampled images
        sampled_images = diffusion.p_sample_loop(model, batch_size=images.shape[0])
        save_images(
            images=sampled_images, 
            path=os.path.join("resultsjob", experiment_name, f"{epoch}.jpg"),
            show=show, 
            title=f'Epoch {epoch}'
        )

        # Every 50 epochs: save intermediate denoising steps
        if epoch % 50 == 0:
            timesteps_to_save = list(range(diffusion.T, 0, -diffusion.T // 8))  # save 8 frames spread across denoising
            _, intermediate_steps = diffusion.p_sample_loop(
                model, batch_size=images.shape[0], timesteps_to_save=timesteps_to_save
            )
            for i, im in enumerate(intermediate_steps):
                save_images(
                    images=im, 
                    path=os.path.join("resultsjob", experiment_name, f"{epoch}_step_{i}.jpg"),
                    show=False,
                    title=f"Denoising Step {i} (Epoch {epoch})"
                )
