...
def create_result_folders(experiment_name):
    os.makedirs("mulan_models_old", exist_ok=True)
    os.makedirs("mulan_results_old", exist_ok=True)
    os.makedirs(os.path.join("models_old", experiment_name), exist_ok=True)
    os.makedirs(os.path.join("results_old", experiment_name), exist_ok=True)

def train(device='cpu', T=500, img_size=64, input_channels=3, channels=32, time_dim=256,
          batch_size=4, lr=1e-5, num_epochs=200, experiment_name="ddpm", show=False):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        save_images(images=sampled_images, path=os.path.join("mulan_results_old", experiment_name, f"{epoch}.jpg"),
                    show=show, title=f'Epoch {epoch}')
        torch.save(model.state_dict(), os.path.join("models_old", experiment_name, f"mulan-weights-{epoch}.pt"))
