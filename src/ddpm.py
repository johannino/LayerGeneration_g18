...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        if schedule == 'linear':
            ...
            return self.cosine_beta_schedule(self.T) # TODO: implement cosine schedule
        # elif schedule == 'quadratic':
        #     return quadratic_beta_schedule(self.T) # TODO: implement quadratic schedule
    
        # add your own (e.g. cosine)
        else :
            raise NotImplementedError('Not implemented!')
    
    def cosine_beta_schedule(self, T):
        """
        Cosine schedule for betas
        """
        betas = torch.linspace(0, 1, T + 1)
        betas = betas[1:] - betas[:-1]
        betas = betas * (1 - torch.cos(torch.linspace(0, np.pi, T))) / 2
        return betas
    ...
    def q_sample(self, x, t):
        ...
        sqrt_alpha_bar =  torch.sqrt(self.alphas_bar[t]) # HINT: get the value of alpha_bar at timestep t
        ...
        sqrt_one_minus_alpha_bar = torch.sqrt(1 - self.alphas_bar[t]) # HINT: get the value of 1 - alpha_bar at timestep t
        ...
        x_noised =  sqrt_alpha_bar * x + sqrt_one_minus_alpha_bar * noise # HINT: calculate the noised image using the forward process
    ...
    def p_mean_std(self, model, x_t, t):
        ...
        predicted_noise =  model(x_t, t) # HINT: use the model to predict the noise. See Eq. 11 in the ddpm paper at page 4
    ...
    def p_sample(self, model, x_t, t):
        ...
        noise = torch.randn_like(x_t) # HINT: sample noise from a normal distribution. It should match the shape of x_t

        x_t_prev =  mean + std * noise # HINT: sample from p(x_{t-1} | x_t) using the mean and std. See Eq. 11 in the ddpm paper at page 4
